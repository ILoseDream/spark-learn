...
public class DailyTop3Keyword {
  ...
  public static void main(String[] args) {
    ...
    queryParamMap.put("city", Lists.newArrayList("beijing"));
    queryParamMap.put("platform", Lists.newArrayList("android"));
    queryParamMap.put("version", Lists.newArrayList("1.0", "1.2", "1.5", "2.0"));

    // 根据我们实现思路中的分析，这里最合适的方式，是将该查询参数Map封装为一个Broadcast广播变量
    // 这样可以进行优化，每个Worker节点，就拷贝一份数据即可
    Broadcast<Map<String, List<String>>> queryParamMapBroadcast = sparkContext.broadcast(
        queryParamMap);

    // 针对HDFS文件中的日志，获取输入RDD
    JavaRDD<String> rawRDD = sparkContext.textFile(
        "hdfs://spark1:9000/spark-study/keyword.txt");
    ...
    JavaPairRDD<String, Long> dateKeywordUvRDD = dateKeywordUsersRDD.mapToPair(
        dateKeywordUsers -> {
          ...
          // 对用户进行去重，并统计去重后的数量
          Set<String> distinctUsers = Sets.newHashSet();
          while (users.hasNext()) {
            String user = users.next();
            distinctUsers.add(user);
          }
          return new Tuple2<>(dateKeyword, (long) distinctUsers.size());
        });

    // 将每天每个搜索词的uv数据，转换成DataFrame
    JavaRDD<Row> dateKeywordUvRowRDD = dateKeywordUvRDD.map(v1 -> {
    ...
    DataFrame dailyTop3KeywordDF = hiveContext.sql("SELECT date,keyword,uv "
        + "FROM ("
        + "SELECT "
        + "date,"
        + "keyword,"
    ...
    JavaRDD<Row> sortedRowRDD = sortedUvDateKeywordsRDD.flatMap(tuple -> {
      ...
      rows.add(RowFactory.create(date,
          dateKeywordsSplit[1].split("_")[0],
          Long.valueOf(dateKeywordsSplit[1].split("_")[1])));
      rows.add(RowFactory.create(date,
          dateKeywordsSplit[2].split("_")[0],
          Long.valueOf(dateKeywordsSplit[2].split("_")[1])));
      rows.add(RowFactory.create(date,
          dateKeywordsSplit[3].split("_")[0],
